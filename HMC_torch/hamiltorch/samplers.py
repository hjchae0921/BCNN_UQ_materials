...
from . import util
import os 
import architectures.u_net.layers.metrics as metrics
from sklearn.metrics import mean_squared_error
import numpy as np
from utils.utils_process import comp_tau_list, construct_paths, get_filepath, get_largest_params
...
def leapfrog(params, momentum, log_prob_func, steps=10, step_size=0.1, jitter=0.01, normalizing_const=1., softabs_const=1e6, explicit_binding_const=100, fixed_point_threshold=1e-20, fixed_point_max_iterations=6, jitter_max_tries=10, inv_mass=None, ham_func=None, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, store_on_GPU = True, debug=False, pass_grad = None):
    ...
    if sampler == Sampler.HMC and integrator != Integrator.SPLITTING and integrator != Integrator.SPLITTING_RAND and integrator != Integrator.SPLITTING_KMID:
        def params_grad(p):
            ...
            log_prob = log_prob_func(p,train=True)
            # log_prob.backward()
...
def hamiltonian(params, momentum, log_prob_func, jitter=0.01, normalizing_const=1., softabs_const=1e6, explicit_binding_const=100, inv_mass=None, ham_func=None, sampler=Sampler.HMC, integrator=Integrator.EXPLICIT, metric=Metric.HESSIAN):
    ...
    if sampler == Sampler.HMC:
        if type(log_prob_func) is not list:
            log_prob = log_prob_func(params,train=True)
...
def sample(log_prob_func, params_init, num_samples=10, num_steps_per_sample=10,  num_samples_start = 0, num_samples_end = 100, step_size=0.1, burn=0, jitter=None, inv_mass=None, normalizing_const=1., softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, debug=False, desired_accept_rate=0.8, store_on_GPU = True, pass_grad = None, verbose = False, weight_path = None, X_batch = None, Y_batch = None, net = None, data_path = None, tau=None, cfg = None, max_indices = None):
    ...
    for n in range(num_samples_start, num_samples_end):        
    # for n in range(num_samples):
        if not verbose:
            util.progress_bar_update(n)
        try:
            #!!!!
            pred_and_save_output(net, X_batch, Y_batch, ret_params, tau, device, data_path, n, cfg, max_indices)
            #!!!!
        ...
        if weight_path is not None:
            dump_path = generate_path(weight_path, n)
            with open(dump_path, 'wb') as f:
                pickle.dump(ret_params[-1:], f)
...
def compute_loss(output, y_sample, model_loss, crit, tau_out):
        if model_loss == 'binary_class_linear_output':
            crit = nn.BCEWithLogitsLoss(reduction='sum')
            return -tau_out * crit(output, y_sample)
        elif model_loss == 'multi_class_linear_output':
            crit = nn.CrossEntropyLoss(reduction='sum')
            return -tau_out * crit(output, y_sample.long().view(-1))
        elif model_loss == 'multi_class_log_softmax_output':
            return -tau_out * torch.nn.functional.nll_loss(output, y_sample.long().view(-1))
        elif model_loss == 'regression':
            return -0.5 * tau_out * ((output - y_sample) ** 2).sum(0)
        elif callable(model_loss):
            return -0.5 * tau_out * model_loss(output, y_sample).sum()
        else:
            raise NotImplementedError()

def define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=1., predict=False, prior_scale = 1.0, device = 'cpu', cfg = None, data_path=None, trainloader = None):
    ...
    pretrained_weights = [param.data for param in model.parameters()]
    # Initialize dist_list with the pretrained weights as means
    dist_list = []
    for weight, tau in zip(pretrained_weights, tau_list):
        mean = weight  # Use the pretrained weight as the mean
        std_dev = tau**-0.5  # Standard deviation from precision
        dist_list.append(torch.distributions.Normal(mean, std_dev))

    # define the 
    def log_prob_func(params,train=False):
        # model.zero_grad()
        # params is flat
        ...
        for weights, index, shape, dist in zip(model.parameters(), params_flattened_list, params_shape_list, dist_list):
            w = params[i_prev:index+i_prev].reshape(shape)  # Reshape w
        ...
        if train:
            x_path = data_path + "/X_tr_sam.npy"
            y_path = data_path + "/X_tr_sam.npy"
            # print(cfg.device)
            x_sample = torch.from_numpy(np.load(x_path).astype(np.float32)).to('cpu')
            y_sample = torch.from_numpy(np.load(y_path).astype(np.float32)).to('cpu')
            dataset = TensorDataset(x_sample, y_sample)
            # data_loader = DataLoader(dataset, batch_size=cfg.batch_size, shuffle=False)
            x_batch = x.to(device)
            y_batch = y.to(device)
            output = fmodel(x_batch, params=params_unflattened)
            crit = []
            ll = compute_loss(output, y_batch, model_loss, crit, tau_out)
            accumulated_loss += ll
            total_loss = (accumulated_loss + l_prior / prior_scale)
        if predict:
            # print(accumulated_loss)
            # print(accumulated_loss)/(l_prior / prior_scale))
...
def define_split_model_log_prob(model, model_loss, train_loader, num_splits, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=1., predict=False, device = 'cpu', verbose = False, cfg = None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        if batch_idx > num_splits - 1:
            break
        log_prob_func = define_model_log_prob(model, model_loss, data.clone().to('cpu'), target.clone().to('cpu'), params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=normalizing_const, prior_scale = num_splits, predict = predict, device = device, cfg = cfg)
...
def sample_model(model, x, y, params_init, model_loss='multi_class_linear_output' ,num_samples=10, num_samples_start = 0, num_samples_end = 100,num_steps_per_sample=10, step_size=0.1, burn=0, inv_mass=None, jitter=None, normalizing_const=1., softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, debug=False, tau_out=1.,tau_list=None, store_on_GPU = True, desired_accept_rate=0.8, verbose = False,weight_path=None, X_batch=None, Y_batch=None, net=None, data_path = None,tau=None, cfg = None, max_indices = None, trainloader = None):
    ...
    log_prob_func = define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=normalizing_const,  device = device, cfg = cfg, data_path = data_path, trainloader = trainloader)

    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    return sample(log_prob_func, params_init, num_samples=num_samples, num_steps_per_sample=num_steps_per_sample, num_samples_start=num_samples_start, num_samples_end=num_samples_end, step_size=step_size, burn=burn, jitter=jitter, inv_mass=inv_mass, normalizing_const=normalizing_const, softabs_const=softabs_const, explicit_binding_const=explicit_binding_const, fixed_point_threshold=fixed_point_threshold, fixed_point_max_iterations=fixed_point_max_iterations, jitter_max_tries=jitter_max_tries, sampler=sampler, integrator=integrator, metric=metric, debug=debug, desired_accept_rate=desired_accept_rate, store_on_GPU = store_on_GPU, verbose = verbose, weight_path = weight_path, X_batch = X_batch, Y_batch = Y_batch, net = net, data_path = data_path,tau=tau, cfg=cfg, max_indices = max_indices)
...
def predict_model(model, samples, x = None, y = None, test_loader = None, model_loss='multi_class_linear_output', tau_out=1., tau_list=None, verbose=False, cfg = None, data_path = None):
    ...
    with torch.no_grad():
        ...
        if test_loader.__class__ is torch.utils.data.dataloader.DataLoader:
            ...
            log_prob_list = define_split_model_log_prob(model, z, test_loader, num_batches, params_flattened_list, params_shape_list, tau_list, tau_out, normalizing_const=1., predict=True, device = samples[0].device, verbose = verbose, cfg=cfg)
            ...
            log_prob_func = define_model_log_prob(model, model_loss, x, y, params_flattened_list, params_shape_list, tau_list, tau_out, predict=True, device = samples[0].device, cfg = cfg,data_path=data_path)            
...
def pred_and_save_output(net, X_batch, Y_batch, ret_params, tau, device, data_path, i, cfg, max_indices):
     # run a direct simulation at each network sample so you can monitor the error
    X_batch = X_batch.to(device)
    Y_batch = Y_batch.to(device) 
    tau_list = comp_tau_list(net, tau, device)
    ret_params = [param.to(device) for param in ret_params]
    ret_params_last = ret_params[0]
    ret_params_max = ret_params_last[max_indices]
    ret_params_max = ret_params_max.cpu()
    # Monitor the largest parameters of the network
    _, _, train_path = construct_paths(cfg)
    os.makedirs(train_path, exist_ok=True)
    save_path = os.path.join(train_path, f'max_params_{i}_HMC.pkl')
    with open(save_path, 'wb') as file:
        pickle.dump(ret_params_max, file)
    # Use the tensor for prediction
    fmodel = util.make_functional(net)
    params_unflattened = util.unflatten(net, ret_params[0])
    output = fmodel(X_batch, params=params_unflattened)
    # Create if it does not exist
    _, _, train_path  = construct_paths(cfg)
    mse_path = os.path.join(train_path, "mse_error_hmc.pkl")
    if os.path.exists(mse_path):
        with open(mse_path, 'rb') as mse_file:
            mse_error = pickle.load(mse_file)
    else:
    ...
    output_path = os.path.join(train_path, f"out_{i}.pkl")
